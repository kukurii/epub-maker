...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, onExport, onReset, isOpen, onClose }) => {
  ...
  const handleResetClick = async () => {
    ...
    if (await dialog.confirm('⚠️ 警告：确定要清空所有数据吗？\n\n此操作将删除所有章节、图片、样式和设置，且无法撤销。\n页面将会刷新以完成重置。')) {
      onReset();
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center px-4 py-3 rounded-xl transition-all duration-200 text-sm font-medium ${currentView === item.id
